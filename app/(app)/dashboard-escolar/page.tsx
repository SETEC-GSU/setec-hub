...
function calcMetrics(e: any) {
  ...
  const equipInativos =
    Math.max(equipRecebidos - equipFuncionando, 0)
...
export default function DashboardPage() {
  ...
  const totalAlunos = escolas.reduce((acc, e) => acc + e.alunos, 0)

  const totalEquipFuncionando =
    escolas.reduce((acc, e) => acc + e.equipFuncionando, 0)

  const totalEquipRecebidos =
    escolas.reduce((acc, e) => acc + e.equipRecebidos, 0)

  const totalEquipIdeal =
    escolas.reduce((acc, e) => acc + e.equipIdeal, 0)

  const deficitEquip =
    Math.max(totalEquipIdeal - totalEquipFuncionando, 0)

  const totalAP =
    escolas.reduce((acc, e) => acc + e.aps, 0)

  const totalSalas =
    escolas.reduce((acc, e) => acc + e.salas, 0)

  const scoreMedio =
    escolas.reduce((acc, e) => acc + e.score, 0) / escolas.length

  const coberturaEquip =
    totalEquipIdeal ? totalEquipFuncionando / totalEquipIdeal : 0

  const totalWifiIdeal =
    escolas.reduce((acc, e) => acc + e.wifiIdeal, 0)

  const coberturaWifi =
    Math.min(totalAP / (totalWifiIdeal || 1), 1)

  const equipamentosParados =
    totalEquipRecebidos - totalEquipFuncionando

  const alunosPorEquip =
    totalEquipFuncionando ? totalAlunos / totalEquipFuncionando : 0

  const salasPorAP =
    totalAP ? totalSalas / totalAP : 0

  const criticas =
    escolas.filter((e) => e.criticidade === "critica").length

  const atencao =
    escolas.filter((e) => e.criticidade === "atencao").length

  const saudavel =
    escolas.filter((e) => e.criticidade === "saudavel").length

  // ranking por score
  const rankingScore =
    [...escolas].sort((a, b) => a.score - b.score).slice(0, 10)

  // ranking por MAU USO
  const rankingMauUso =
    [...escolas].sort((a, b) => b.equipInativos - a.equipInativos).slice(0, 10)
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-slate-400 text-sm">
          Panorama tecnológico das Escolas da URE
        </p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
        ...
        <Card title="Déficit equipamentos" value={deficitEquip} />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card
          title="Cobertura equipamentos"
          value={`${(coberturaEquip * 100).toFixed(0)}%`}
        />

        <Card
          title="Cobertura Wi-Fi"
          value={`${(coberturaWifi * 100).toFixed(0)}%`}
        />
        ...
        <Card
          title="Alunos por equipamento"
          value={alunosPorEquip.toFixed(1)}
        />
      ...
      <div className="bg-[#020617] border border-slate-800 rounded-2xl p-6">
        ...
        <h3 className="text-lg font-semibold mb-4">
          Escolas com maior taxa de mau uso de equipamentos
        </h3>
        ...
        <div className="grid md:grid-cols-2 gap-3">
          ...
          {rankingMauUso.map((e) => (
            ...
            >
              ...
              <p className="text-xs text-slate-400">
                {e.equipInativos} equipamentos não operacionais
              </p>
      ...
      <div className="bg-[#020617] border border-slate-800 rounded-2xl p-6 space-y-2">
        ...
        <p className="text-sm text-slate-400">
          A URE possui déficit estimado de
          <b> {deficitEquip} equipamentos</b>
          considerando a proporção ideal de
          1 equipamento para cada 3 alunos.
        </p>

        <p className="text-sm text-slate-400">
          Existem atualmente
          <b> {equipamentosParados}</b> equipamentos instalados mas não operacionais.
        </p>
